...
dados_alimentos = pd.DataFrame({
    "Alimento": ["Chocolate", "Chocolate", "Chocolate", "Chocolate"],
    "Resumo": ["Informações rápidas do produto.", "Informações rápidas do produto.", "Informações rápidas do produto.", "Informações rápidas do produto."],
})
...
if st.session_state['pagina_atual'] == 'home':
    ...
    <style>
        ...
        /* 1. Miramos no kind="primary" com as regras separadas para não falhar */
        button[kind="primary"], 
        ...
        /* 2. Destrói todo o texto e elementos injetados DENTRO do botão */
        button[kind="primary"] * {{
            display: none !important;
        }}
        
        /* 3. Destrói o efeito de piscar no momento exato do clique */
        button[kind="primary"]::before, 
        button[kind="primary"]::after {{
            display: none !important;
        }}
        
        /* 4. Efeito suave de zoom */
        button[kind="primary"]:hover {{ 
    ...
    with col_centro:
        # Passamos o texto normalmente, o CSS faz o resto
        if st.button("", type="primary", use_container_width=True):
            st.session_state['pagina_atual'] = 'catalogo'
            st.rerun()

    # Adicionamos o ícone de informação direto na string
